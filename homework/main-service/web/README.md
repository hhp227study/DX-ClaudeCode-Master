...
- **채보 좌표는 몸 기준** (2026-07-22) — 그전까지 채보 x/y를 캔버스 폭·높이에 그대로 곱해서,
  ...
  (`engine/chart.ts remapChartToBody`). 픽셀 기준 계산이라 세로 위치는 기기와 무관하게 동일하다.
  **가로는 화면이 허용하는 만큼 벌린다(X_REACH 2.0 상한)** — 저작 x 한계(0.10~0.90)가 원래
  "버블이 화면 밖으로 안 나가게"라는 화면 기준 값이라 몸 단위로는 ±0.57 어깨너비(완전히 뻗은
  팔의 27%)밖에 안 됐다. 셀카 프레임이라 팔을 오므리고 찍은 탓인데 게임은 카메라가 더 멀다.
  지금은 PC 가로에서 ±1.14 어깨너비까지 쓰고, 세로 폰에서는 화면 밖으로 나가지 않게 좁힌다.
  화면 회전 시 resize에서 다시 맞춘다.

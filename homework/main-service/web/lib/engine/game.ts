...
/** 판정이 발생한 제스처 종류 — Analytics gesture_detected 파라미터 (PRD 12.1) */
export type GestureKind = 'catch' | 'decoy' | 'hands_up' | 'heart';
...
export interface GameResult {
  ...
  rank: string;
  maxCombo: number;
  /** 최대 콤보를 달성한 곡 시각(ms) — 하이라이트 구간의 중심 (FR-15) */
  maxComboAtMs: number;
  fullCombo: boolean;
...
export class ChartGame {
  ...
  running = false;
  /** 캘리브레이션/설정에서 측정·조정된 판정 오프셋(ms). 양수 = 입력이 늦게 도착하는 기기 (FR-05) */
  offsetMs = 0;
  score = 0;
  combo = 0;
  maxCombo = 0;
  private maxComboAtMs = 0;
  ...
  update(now: number, cursors: PxPoint[], pose: PoseFlags | null, w: number, h: number): void {
    ...
    this.live = this.live.filter((n) => {
      const def = n.def;
      // catch 노트의 타이밍 판정에만 오프셋 적용 (스폰·포즈·함정 수명은 원시 클록)
      const dt = def.type === 'catch' ? now - def.t - this.offsetMs : now - def.t;
      ...
      if (def.type === 'pose') {
        ...
        if (dt > dur) {
          ...
          this.judge(grade, 0.5, 0.35, now, def.pose ?? 'hands_up');
          return false;
        }
        return true;
      }
      ...
      if (def.type === 'decoy') {
        if (touched && dt >= -APPROACH_MS) {
          this.judge('DECOY', def.x ?? 0.5, def.y ?? 0.5, now, 'decoy');
          return false;
        }
  ...
  private judge(grade: Grade, x: number, y: number, now: number, gesture: GestureKind): void {
    this.counts[grade]++;
    if (grade === 'MISS') {
      this.combo = 0;
    } else if (grade === 'DECOY') {
  ...
  finish(): GameResult {
    ...
    return {
      ...
      maxComboAtMs: this.maxComboAtMs,
      fullCombo,
      counts: { ...c },
    };
  }

...
- 2026-07-22 세션: **카라멜단센을 실제 음원(MP3)으로 교체 + 채보 재작성**
  ...
  ⑩ **점수가 항상 F로 나오던 문제 수정** (유저 신고) — 임계값 문제가 아니라 판정 모델의 결함이었다.
     버블은 2초 내내 고정 위치에 그려지고 링만 좁혀지므로 "손을 대고 링이 닫히길 기다리는" 것이
     정상 플레이인데, 판정은 접촉 즉시 이뤄져 윈도우가 열리는 첫 순간(dt=-250ms)에 걸렸다
     → 무조건 GOOD(가중치 0.3) → 전부 잡아도 30%대 → F. 시각 신호와 판정이 모순이었다.
     수정: 판정 시각 전의 접촉은 "가장 가까웠던 오차"로 기억만 하고, dt>=0을 지나는 순간 판정한다
     (미리 대고 기다리면 PERFECT). 미리 닿았다 뗀 경우는 최선 접근으로 인정.
     실채보 시뮬레이션(30fps): 95% 캐치 시 29.8% F → 92.7% SS, 80% 캐치 시 A, 60% 캐치 시 B.
     함께 완화 — 가중치 GREAT 0.7→0.85·GOOD 0.3→0.5, 랭크 경계 하향(SSS96/SS90/S82/A72/B55/C35).
     랭크 종류는 DB CHECK 제약(plays.rank 7종)과 맞춰 그대로 둠
  ⑪ **녹화 영상 무음 문제 수정** (유저 신고) — canvas.captureStream에는 오디오가 없어서
     처음부터 무음이었다. AudioContext master에서 MediaStreamDestination으로 갈라내
     (`captureFrom`, 스피커 출력은 유지) 캔버스 비디오 트랙과 합쳐 녹화한다.
     하이라이트 클립도 재인코딩 경로라 같은 문제였다 — 원본 <video>를 MediaElementSource로
     그래프에 끌어오되 ctx.destination에는 연결하지 않아, 클립을 만드는 동안 유저에게는
     여전히 무음이면서 녹음에는 소리가 담긴다. mime 후보에 opus/mp4a 코덱 명시.
     ⚠️ track.stop()을 녹화 종료 뒤로 옮겼다 — 먼저 내리면 오디오 트랙이 끝나 엔드카드가 잘린다.
     검증(Chrome headless): mime `video/webm;codecs=vp9,opus`, 오디오 트랙 1개,
     결과 blob 디코딩 RMS 0.2879(무음이면 0)

  ⓬ **모바일/PC 채보 불일치 수정** (유저 지적) — 채보 x/y를 캔버스 폭·높이에 그대로 곱하던 탓에
     세로 폰과 가로 PC에서 같은 채보가 몸 기준으로 4배 다른 위치에 떴다(앵커가 차지하는 카메라
     화각 11% vs 44%, 폰에선 어깨가 화면 밖). 유저 요구는 "PC는 가로 그대로" —
     letterbox 대신 좌표를 몸 기준으로 바꿨다: 시작 직전 어깨 중심·너비를 샘플링해 몸 앵커를 잡고
     저작 기준(어깨너비 0.70·어깨선 y 0.55, 9:16)의 어깨너비 배수 오프셋을 실제 픽셀로 환산
     (`remapChartToBody`). 검증: 폰 세로/PC 가로/창모드/태블릿 전부 몸기준 x±0.31·y+0.08/−0.15로 동일.
     화면 회전은 resize에서 재적용. 프레이밍(클로즈업/풀바디) 설정은 인식 실패 시 폴백으로 강등
